...
const f=e.target.files[0]; if(!f)return;
try{
const text=await new Promise((r,j)=>{const rd=new FileReader();rd.onload=ev=>r(ev.target.result);rd.onerror=j;rd.readAsText(f);});
...
// 📋 Quote workspace file
$('btnQuote').onclick=async()=>{
const cwd=($('fCwd').value||'.').replace(/~\?/,'');
try{
const files=await inv('list_files',{dir:cwd,max:100});
const list=$('wsList');
list.innerHTML='';
if(!files||files.length===0){list.innerHTML='<div id="wsEmpty">No files found</div>';}
else{
files.forEach(f=>{
if(!f.is_file)return;
const row=document.createElement('div');row.className='row';
row.innerHTML='<span class="n">'+escHtml(f.name)+'</span><span class="s">'+fmtSize(f.size)+'</span>';
row.onclick=async()=>{
try{const txt=await inv('read_file',{path:f.path,max_bytes:8000});
const q='[quote '+f.name+']\n'+txt.slice(0,6000)+'\n[/quote]\n';
$('inp').value+=$('inp').value?'\n'+q:q;
$('inp').focus();
wsClose();
...
}catch(err){alert('List error: '+err);}
};
$('wsClose').onclick=wsClose;
$('wsOverlay').onclick=e=>{if(e.target===$('wsOverlay'))wsClose();};
function wsClose(){$('wsOverlay').classList.remove('on');}

// Send: support content_blocks when message has [img data-url=...] 
$('btnSend').onclick=send;$('inp').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}});
async function send(){
let c=$('inp').value.trim(); if(!c)return; $('inp').value='';
addMsg('user',c.length>200?c.slice(0,200)+'…':c);
try{
// Check if message contains image data-url
const imgMatch=c.match(/\[img data-url=([\s\S]*?)\]/);
if(imgMatch){
const dataUrl=imgMatch[1].trim();
const text=c.replace(/\[img data-url=[\s\S]*?\]/,'').trim();
const blocks=[{type:'image_url',image_url:{url:dataUrl}}];
if(text)blocks.push({type:'text',text});
await inv('send_prompt',{sessionId:'chat',content_blocks:blocks});
}else{
await inv('send_prompt',{sessionId:'chat',content:c});
}
}catch(e){addMsg('assist','Error: '+e);}
}
